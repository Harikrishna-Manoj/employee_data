...
class _DatePickerDialogState extends State<DatePickerDialog>
  ...
  Widget build(BuildContext context) {
      ...
      switch (_entryMode.value) {
        ...
        case DatePickerEntryMode.calendarOnly:
          if (orientation == Orientation.portrait) {
            headlineStyle = textTheme.bodyMedium;
          }
    ...
    final Widget actions = Container(
      ...
      // decoration:
      //     BoxDecoration(border: Border.all(color: calendarDividerGreyColor)),
      child: OverflowBar(
        spacing: 16,
        children: <Widget>[
          const Icon(
...
/// This is used by the [_MonthPicker] to let its children [_DayPicker]s know
/// what the currently focused date (if any) should be.
class _FocusedDatea extends InheritedWidget {
  const _FocusedDatea({
    required super.child,
    this.date,
  });
...
class _DayPickerState extends State<_DayPicker> {
  ...
  void didChangeDependencies() {
    super.didChangeDependencies();
    // Check to see if the focused date is in this month, if so focus it.
    final DateTime? focusedDate = _FocusedDatea.maybeOf(context);
    if (focusedDate != null &&
        DateUtils.isSameMonth(widget.displayedMonth, focusedDate)) {
      _dayFocusNodes[focusedDate.day - 1].requestFocus();
    }
  ...
  List<Widget> _dayHeaders(
      TextStyle? headerStyle, MaterialLocalizations localizations) {
    final List<Widget> result = <Widget>[];
    final List<String> shortDays = <String>[
      "Sun",
      "Mon",
      "Tue",
      "Wed",
    ...
    for (int i = localizations.firstDayOfWeekIndex;
        result.length < DateTime.daysPerWeek;
        i = (i + 1) % DateTime.daysPerWeek) {
      final String weekday = shortDays[i];
      result.add(ExcludeSemantics(
        child:
            Center(child: Text(weekday, style: const TextStyle(fontSize: 15))),
      ));
    }
    return result;
  }
